...
export class Game extends BaseGame {
	...
	public async start() {
		...
		// таймаут добавлен для того чтобы подолше задержаться на лого, и увидеть все его анимации
		await gsapSleep(4000);
		await this.startPreloader();

		// таймаут добавлен для того чтобы подолше задержаться на прелоадере, и увидеть часть его анимаций
		await gsapSleep(4000);
	...
	private async startPreloader() {
		...
		await this.preloader?.initPogressWatcher();
	}
